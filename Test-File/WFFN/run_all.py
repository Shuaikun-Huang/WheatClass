...
test_num = 2;
#测试网络
deploy = 'D:/Profiles/Huang-project/WheatClass/Test-File/WFFN/WFFN_deploy.prototxt'
#模型地址
model_dir = 'D:/Profiles/Huang-project/WheatClass/Test-File/WFFN/snapshot/'
#分的类
...
for j in range(len(files)):
    ...
    for i in range(0, test_num):
        ...
        #预测分类文件存储位置即名字
        predict_result_txt = "D:/Profiles/Huang-project/WheatClass/Test-File/WFFN/predict_result_txt/%s_%d.txt"%(files[j][:-11],i)
        #分类结果矩阵等文件地址及名字
        result =  "D:/Profiles/Huang-project/WheatClass/Test-File/WFFN/result/%s_%d.txt"%(files[j][:-11],i)
        caffe.set_mode_gpu()
        caffe.set_device(0)
